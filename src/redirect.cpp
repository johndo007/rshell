...
int setup_task0(struct str_direct &r)
{
	...
	if(r.task_count>0)
	{
		if(dup2(r.pipes[r.task_count*2-1],1)<0)perror("Error: dup2pipe");	//1 means output to next task
		redirect_out=1;
	}
	...
	if(found_out)
	{
		...
		}else if(strcmp(r.current_task[s],">>")==0)
		{
			r.out=open(r.current_task[s+1],O_WRONLY|O_APPEND, S_IRUSR|S_IRGRP|S_IWGRP|S_IRGRP|S_IWGRP|S_IWUSR);
			if(r.out<0)	perror("open");
		}
